...
#include <string.h>
#include <ctype.h>
//constantes simbolicas para me ajudar
#define VAR_LENGTH 6
#define EPSILON 1e-9
...
int simplex(tableau_t *tableau, int two_phases) {
  ...
  if (two_phases) {
    printf("\n\n# Comeco da primeira fase:\n\n");
  } else {
    printf("\n\n# Comeco da segunda fase:\n\n");
  }
...
int main(int argc, char *argv[]) {
  ...
  tableau.rows += (tableau.art_vars > 0) ? 2 : 1;
  ...
  // adiciona variaveis artificiais
  ...
  strcpy(tableau.cols_h[++c], "sol"); // adiciona a solucao
  ...
  if (rs != 0) {
    printf("\n\nnao foi possivel encontrar a solucao otima!\n");
    return 1;
  }
